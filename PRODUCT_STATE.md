...
- **AI** — Anthropic only (`claude-haiku-4-5-20251001`), used for four distinct features: natal Analysis, Synastry analysis, "Personal Synthesis" (single placement, 3-4 sentences), and Chat. All four draw from one shared daily quota (10 calls/day per user or per guest device).
- **Persistence** — deliberately minimal. Chart data itself lives client-side only (`localStorage` in this repo, keyed by chart id — see `lib/storage.ts`). The backend has a JSON-file store for cross-device chart recall and a JSON-file rate limiter; both are explicitly documented in-code as scope-limited placeholders, not real databases.
...
- **Backend**: 246 tests across 11 files (up from 144 in mid-July, tracking real feature growth — Synastry, Transits, Chat, and the shared quota endpoint all landed since then). Heavily concentrated in Electional (94 tests) and Synastry (27).
...
1. **Electional Pro badges aren't enforced client-side.** A free user can click straight through a Pro-marked theme today; the badge is currently decoration.
2. **Checkout is explicitly "coming soon."** No live purchase path yet.
3. **No account/settings page exists.** The nav avatar is decorative — no dropdown, no link, nothing behind it.
